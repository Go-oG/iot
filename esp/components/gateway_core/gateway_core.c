...
#define GW_MQTT_SUPERVISOR_POLL_MS 1000
#define GW_MQTT_RECONNECT_AFTER_MS 15000
#define GW_MQTT_STUCK_RESTART_MS 60000
#define GW_MQTT_DEAD_RESTART_MS 5000
#define GW_MQTT_KICK_INTERVAL_MS 10000
#define GW_MQTT_START_RETRY_MS 5000
...
static SemaphoreHandle_t g_mqtt_lock = NULL;
static unsigned g_mqtt_restarts = 0;

static bool mqtt_lock(TickType_t timeout) {
    return g_mqtt_lock && xSemaphoreTakeRecursive(g_mqtt_lock, timeout) == pdTRUE;
}

static void mqtt_unlock(void) {
    if (g_mqtt_lock)
        xSemaphoreGiveRecursive(g_mqtt_lock);
}
...
static bool config_load(gateway_config_t *cfg) {
    ...
    if (ready == 1) {
        ...
        cfg->configured = cfg->wifi_ssid[0] != '\0' && mqtt_uri_valid(cfg->mqtt_uri) && gateway_id_valid(
                              cfg->gateway_id);
    }
    nvs_close(nvs);
...
static const char CONFIG_PAGE[] =
        ...
        "<script>let current={},seq=0;"
        "async function api(op,data){let req={v:1,type:'req',reqId:'cfg-'+Date.now()+'-'+(++seq),op:op,data:data};"
        "let r=await fetch('/api/v1',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(req)});let j=await r.json();"
        "if(!r.ok||j.code)throw Error(j.message||'请求失败');return j.data||{};}"
        "async function load(){try{current=await api('manage',{action:'config.get'});ssid.value=current.wifiSsid||'';uri.value=current.mqttUri||'';user.value=current.mqttUsername||'';gid.value=current.gatewayId||'';}catch(e){msg.textContent='加载失败：'+e.message;}}"
        ...
        "try{await api('manage',{action:'config.set',config:b});msg.textContent='保存成功，设备正在重启。';}catch(e){msg.textContent='保存失败：'+e.message;}}load();</script></body></html>";
...
int gw_adapter_config(const cJSON *config, cJSON **result, const char **error) {
    ...
    if (!json_copy_string(root, "wifiSsid", next.wifi_ssid, sizeof(next.wifi_ssid), true) || next.wifi_ssid[0] ==
        '\0') {
        *error = "invalid_wifi_ssid";
        return 1003;
    }
    if (!json_copy_string(root, "mqttUri", next.mqtt_uri, sizeof(next.mqtt_uri), true) || !
        mqtt_uri_valid(next.mqtt_uri)) {
        *error = "invalid_mqtt_uri";
        return 1003;
    }
    if (!json_copy_string(root, "mqttUsername", next.mqtt_username, sizeof(next.mqtt_username), false)) {
        *error = "invalid_mqtt_username";
        return 1003;
    }
    if (!json_copy_string(root, "gatewayId", next.gateway_id, sizeof(next.gateway_id), true) || !gateway_id_valid(
            next.gateway_id)) {
    ...
    if (next.wifi_password[0] != '\0' && strlen(next.wifi_password) < 8) {
        *error = "wifi_password_must_be_empty_or_8_plus";
        return 1003;
    }
...
static void config_http_start(void) {
    ...
    gw_http_api_register(g_config_httpd);
...
static bool mqtt_output_available_locked(int qos) {
    if (!g_mqtt || !g_runtime_events ||
        (xEventGroupGetBits(g_runtime_events) & MQTT_CONNECTED_BIT) == 0)
        return false;
...
static bool mqtt_publish_json(const char *topic, cJSON *obj, int qos, bool retain) {
    ...
    if (mqtt_lock(portMAX_DELAY)) {
        if (mqtt_output_available_locked(qos)) {
            published = esp_mqtt_client_enqueue(g_mqtt, topic, s, (int) strlen(s), qos, retain, true) >= 0;
        }
        mqtt_unlock();
    }
    cJSON_free(s);
...
static void publish_status(esp_mqtt_client_handle_t client, bool online) {
    if (!client) return;
    cJSON *o = cJSON_CreateObject();
    ...
    if (s) {
        esp_mqtt_client_enqueue(client, g_topic_status, s, (int) strlen(s), 1, true, true);
        cJSON_free(s);
    }
    cJSON_Delete(o);
}
...
static void ble_notify_registration_done(esp_gatt_if_t gattc_if, uint16_t char_handle,
    ...
    if (status == ESP_GATT_OK) {
        uint8_t value[2] = {
            (uint8_t) snapshot.pending_cccd_value,
...
static void ble_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
    switch (event) {
        case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
            ...
            if (duration && (!g_ble_scan_params_ready || esp_ble_gap_start_scanning(duration) != ESP_OK))
        ...
        case ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT: {
            ...
            if (param->read_rssi_cmpl.status == ESP_BT_STATUS_SUCCESS) {
                cJSON *o = event_base("ble.rssi");
                char addr[18];
                bda_to_string(param->read_rssi_cmpl.remote_addr, addr);
                cJSON_AddStringToObject(o, "device", addr);
                cJSON_AddNumberToObject(o, "rssi", param->read_rssi_cmpl.rssi);
                gw_protocol_event(o);
                cJSON_Delete(o);
            }
            break;
        ...
        case ESP_GAP_BLE_SCAN_RESULT_EVT: {
            if (param->scan_rst.search_evt == ESP_GAP_SEARCH_INQ_RES_EVT) {
                                ...
                                param->scan_rst.ble_adv, len);
                {
                    ...
                    char addr[18];
                    bda_to_string(param->scan_rst.bda, addr);
                    cJSON_AddStringToObject(o, "device", addr);
                    ...
                    if (hex) {
                        cJSON_AddStringToObject(o, "advertisement", hex);
                        free(hex);
                    }
                    publish_event(o);
                }
...
static void ble_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
    ...
    switch (event) {
        ...
        case ESP_GATTC_READ_CHAR_EVT: {
            ...
            if (param->read.status == ESP_GATT_OK) {
                ...
                if (hex) {
                    ...
                    cJSON_SetNumberValue(cJSON_GetObjectItemCaseSensitive(o, "status"), ESP_GATT_ERROR);
            }
            publish_event(o);
            break;
        }
        ...
        case ESP_GATTC_NOTIFY_EVT: {
            gw_report_t report = {
                .handle = param->notify.handle, .indicate = !param->notify.is_notify,
            ...
            if (gw_protocol_owns(param->notify.remote_bda)) {
                ...
                char addr[18];
                bda_to_string(param->notify.remote_bda, addr);
                cJSON_AddStringToObject(o, "device", addr);
                ...
                gw_protocol_event(o);
                cJSON_Delete(o);
            } else if (!gw_manager_report(param->notify.remote_bda, &report)) gw_adapter_report(
                param->notify.remote_bda, &report);
            break;
        }
        ...
        case ESP_GATTC_CANCEL_OPEN_EVT: {
            ...
            if (param->cancel_open.status == ESP_GATT_OK) {
                cJSON *o = event_base("ble.connection");
                char addr[18];
                bda_to_string(param->cancel_open.remote_bda, addr);
                cJSON_AddStringToObject(o, "device", addr);
...
cJSON *gw_adapter_diagnostics(void) {
    ...
    cJSON_AddNumberToObject(o, "minimumInternalHeapBytes",
    ...
    int outbox_bytes = 0;
    unsigned mqtt_restarts = 0;
    if (mqtt_lock(portMAX_DELAY)) {
        if (g_mqtt) outbox_bytes = esp_mqtt_client_get_outbox_size(g_mqtt);
        mqtt_restarts = g_mqtt_restarts;
        mqtt_unlock();
    }
    cJSON_AddNumberToObject(o, "mqttOutboxBytes", outbox_bytes);
    cJSON_AddNumberToObject(o, "mqttRestarts", mqtt_restarts);
    int connected = 0, pending = 0;
    portENTER_CRITICAL(&g_connection_lock);
    for (unsigned i = 0; i < GW_MAX_BLE_CONNECTIONS; ++i)
        if (g_ble[i].used) {
            if (g_ble[i].connected) ++connected;
            else ++pending;
        }
    portEXIT_CRITICAL(&g_connection_lock);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t) event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(event->client, g_topic_v1_down, 1);
            gw_protocol_online();
            publish_status(event->client, true);
            break;
        ...
        case MQTT_EVENT_DATA: {
            if (event->current_data_offset == 0) {
                ...
                if (event->topic_len != strlen(g_topic_v1_down) ||
                    ...
                    event->total_data_len <= 0 || event->total_data_len > GW_V1_MAX_FRAME)
                    break;
                total = event->total_data_len;
                payload = malloc((size_t) total);
            }
            if (!payload) break;
            if (event->current_data_offset != received || event->total_data_len != total || event->data_len <= 0 ||
                event->data_len > total - received) {
                free(payload);
                payload = NULL;
                break;
            }
            memcpy(payload + received, event->data, event->data_len);
        ...
        case MQTT_EVENT_ERROR:
            if (event->error_handle) {
                ESP_LOGW(TAG, "MQTT error: type=%d tls=0x%x stack=%d sock_errno=%d",
...
static esp_mqtt_client_handle_t mqtt_client_create(void) {
    char lwt[128];
...
static bool mqtt_client_start(esp_mqtt_client_handle_t client) {
    if (!client || !mqtt_lock(portMAX_DELAY)) return false;
    if (g_mqtt) {
        mqtt_unlock();
        return false;
    }
...
static void mqtt_start(void) {
    ...
    esp_mqtt_client_handle_t client = mqtt_client_create();
    if (client && !mqtt_client_start(client)) {
        ESP_LOGE(TAG, "MQTT initial start failed");
    }
}

static bool mqtt_force_reconnect(void) {
    bool requested = false;
    if (!mqtt_lock(portMAX_DELAY)) return false;
    if (g_mqtt && esp_mqtt_client_get_state(g_mqtt) == MQTT_CLIENT_STATE_WAITING_RECONNECT) {
        requested = esp_mqtt_client_reconnect(g_mqtt) == ESP_OK;
    }
    mqtt_unlock();
    if (requested)
        ESP_LOGW(TAG, "MQTT supervisor requested reconnect");
    return requested;
}

static bool mqtt_restart(const char *reason) {
    ESP_LOGW(TAG, "Restarting MQTT client: %s", reason);
    log_network_memory("Before MQTT restart");
    ...
    if (old) esp_mqtt_client_destroy(old);
    esp_mqtt_client_handle_t fresh = mqtt_client_create();
    if (!fresh || !mqtt_client_start(fresh)) {
        ESP_LOGE(TAG, "MQTT restart failed");
        return false;
    }
    log_network_memory("After MQTT restart");
...
static void mqtt_supervisor_task(void *arg) {
    ...
    int64_t next_kick = 0;
    int64_t next_restart = 0;
    bool was_connected = false;

    while (true) {
        vTaskDelay(pdMS_TO_TICKS(GW_MQTT_SUPERVISOR_POLL_MS));
        EventBits_t bits = xEventGroupGetBits(g_runtime_events);
        ...
        esp_mqtt_client_connection_state_t state = MQTT_CLIENT_STATE_NOT_INITIALIZED;
        if (mqtt_lock(portMAX_DELAY)) {
            has_client = g_mqtt != NULL;
            if (has_client) {
                state = esp_mqtt_client_get_state(g_mqtt);
                connected = state == MQTT_CLIENT_STATE_CONNECTED;
            }
            if (connected) xEventGroupSetBits(g_runtime_events, MQTT_CONNECTED_BIT);
            else xEventGroupClearBits(g_runtime_events, MQTT_CONNECTED_BIT);
            mqtt_unlock();
        }
        ...
        if (connected) {
            if (!was_connected)
                ESP_LOGI(TAG, "MQTT supervisor detected connection");
            offline_since = 0;
            next_kick = 0;
            next_restart = 0;
            was_connected = true;
        ...
        int64_t offline_ms = now - offline_since;

        if (!has_client) {
            if (next_restart == 0 || now >= next_restart) {
                bool restarted = mqtt_restart("client unavailable");
                next_restart = now + (restarted ? GW_MQTT_STUCK_RESTART_MS : GW_MQTT_START_RETRY_MS);
            }
            continue;
        }

        if (state == MQTT_CLIENT_STATE_DISCONNECTED && offline_ms >= GW_MQTT_DEAD_RESTART_MS) {
            bool restarted = mqtt_restart("client stopped");
            offline_since = now;
            next_kick = now + GW_MQTT_KICK_INTERVAL_MS;
            next_restart = now + (restarted ? GW_MQTT_STUCK_RESTART_MS : GW_MQTT_START_RETRY_MS);
            continue;
        }

        if (state == MQTT_CLIENT_STATE_NOT_STARTED && offline_ms >= GW_MQTT_STUCK_RESTART_MS) {
            bool restarted = mqtt_restart("client not started");
            offline_since = now;
            next_kick = now + GW_MQTT_KICK_INTERVAL_MS;
            next_restart = now + (restarted ? GW_MQTT_STUCK_RESTART_MS : GW_MQTT_START_RETRY_MS);
            continue;
        ...
        if (offline_ms >= GW_MQTT_RECONNECT_AFTER_MS && now >= next_kick) {
...
static void mqtt_supervisor_start(void) {
    if (xTaskCreate(mqtt_supervisor_task, "mqtt_supervisor", 4096, NULL, 4, NULL) != pdPASS) {
...
void gateway_start(void) {
    ...
    g_mqtt_lock = xSemaphoreCreateRecursiveMutex();
    if (!g_mqtt_lock) {
        ESP_LOGE(TAG, "Failed to create MQTT lifecycle lock");
        abort();
    }
    ...
    esp_read_mac(mac, ESP_MAC_WIFI_STA);
    char hostname[32];
    snprintf(hostname, sizeof(hostname), "blegw-%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2], mac[3], mac[4],
             mac[5]);
    ...
    if (!configured) {
        ESP_LOGW(TAG, "No runtime configuration. Connect to %s / password '%s' and open http://192.168.4.1",
                 g_config_ap_ssid, g_config_ap_password);
        return;
    }
...
bool gw_adapter_link(const uint8_t addr[6], gw_link_t *link) {
    ...
    *link = (gw_link_t){
        .connected = snapshot.connected, .services_ready = snapshot.services_ready,
...
esp_err_t gw_adapter_connect(const uint8_t addr[6], uint8_t address_type) {
    ...
    for (unsigned i = 0; i < GW_MAX_BLE_CONNECTIONS; ++i)
        if (g_ble[i].used && !g_ble[i].connected) {
            portEXIT_CRITICAL(&g_connection_lock);
...
esp_err_t gw_adapter_subscribe(const gw_device_t *device, unsigned index) {
    ...
    ble_conn_t conn;
    if (!ble_snapshot_by_addr(device->addr, &conn) || !conn.connected || !conn.services_ready) return
    ...
    if (!(ch.properties & (sub->indicate ? ESP_GATT_CHAR_PROP_BIT_INDICATE : ESP_GATT_CHAR_PROP_BIT_NOTIFY))) return
            ESP_ERR_NOT_SUPPORTED;
    esp_bt_uuid_t uuid = {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = ESP_GATT_UUID_CHAR_CLIENT_CONFIG}};
    esp_gattc_descr_elem_t desc;
    uint16_t count = 1;
    if (esp_ble_gattc_get_descr_by_char_handle(g_gattc_if, conn.conn_id, ch.char_handle, uuid, &desc, &count) !=
        ESP_GATT_OK || !count)
        return ESP_ERR_NOT_FOUND;
    if (!ble_begin_cccd_operation(&conn, ch.char_handle, desc.handle, sub->indicate ? 2 : 1, false, true)) return
            ESP_ERR_INVALID_STATE;
    portENTER_CRITICAL(&g_connection_lock);
...
int gw_protocol_ble_start(const cJSON *request, const uint8_t addr[6], uint8_t addr_type,
    ...
    *native_code = 0;
    ...
    if (!strcmp(op, "scan")) {
        if (!strcmp(json_string(data, "action"), "stop")) err = esp_ble_gap_stop_scanning();
            ...
            esp_ble_scan_params_t params = {
                .scan_type = cJSON_IsFalse(cJSON_GetObjectItemCaseSensitive(data, "active"))
    ...
    ble_conn_t conn;
    if (!ble_snapshot_by_addr(addr, &conn) || !conn.connected || !conn.services_ready || conn.cancel_requested) return
            3002;
    if (conn.cccd_pending) return 2001;
    esp_bt_uuid_t svc_uuid, chr_uuid;
    if (!uuid_parse(json_string((cJSON *) request, "service"), &svc_uuid) ||
        !uuid_parse(json_string((cJSON *) request, "char"), &chr_uuid))
        return 1003;
    ...
    if (status != ESP_GATT_OK || !count) {
        *native_code = status;
        return 4001;
    }
    esp_gattc_char_elem_t ch;
    count = 1;
    status = esp_ble_gattc_get_char_by_uuid(g_gattc_if, conn.conn_id, svc.start_handle, svc.end_handle, chr_uuid, &ch,
                                            &count);
    if (status != ESP_GATT_OK || !count) {
        *native_code = status;
        return 4002;
    ...
    *handle = ch.char_handle;
    ...
    if (!strcmp(op, "read")) {
        ...
        if (!(ch.properties & (no_response ? ESP_GATT_CHAR_PROP_BIT_WRITE_NR : ESP_GATT_CHAR_PROP_BIT_WRITE))) return
                failure;
        size_t len = 0;
        ...
        if (len > 512 || len > payload) {
            free(bytes);
            return 1003;
        }
        err = esp_ble_gattc_write_char(g_gattc_if, conn.conn_id, ch.char_handle, len, bytes,
                                       no_response ? ESP_GATT_WRITE_TYPE_NO_RSP : ESP_GATT_WRITE_TYPE_RSP,
        ...
        if (enabled && !(ch.properties & (indicate ? ESP_GATT_CHAR_PROP_BIT_INDICATE : ESP_GATT_CHAR_PROP_BIT_NOTIFY)))
            return failure;
        esp_bt_uuid_t uuid = {.len = ESP_UUID_LEN_16, .uuid = {.uuid16 = ESP_GATT_UUID_CHAR_CLIENT_CONFIG}};
        esp_gattc_descr_elem_t desc;
        count = 1;
        ...
        if (!ble_begin_cccd_operation(&conn, ch.char_handle, desc.handle, enabled ? (indicate ? 2 : 1) : 0, !enabled,
                                      false)) return 2001;
        err = enabled
                  ? esp_ble_gattc_register_for_notify(g_gattc_if, conn.addr, ch.char_handle)
