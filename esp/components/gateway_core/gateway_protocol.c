...
static protocol_value_t *value_evictable(void) {
    ...
    for (unsigned i = 0; i < VALUE_COUNT; ++i) {
        ...
        for (unsigned j = 0; j < REQUEST_COUNT; ++j)
            if (s_requests[j].used && s_requests[j].value_index == (int)i) { busy = true; break; }
        if (busy) continue;
...
typedef struct { int route; } ws_session_t;
static unsigned s_ws_sequence;
typedef struct { int route; char *text; } ws_output_t;
static void ws_send(void *arg) {
    ws_output_t *out = arg;
    int fds[16]; size_t count = 16;
    if (httpd_get_client_list(s_http, &count, fds) == ESP_OK) {
        for (size_t i = 0; i < count; ++i) {
            ...
            if (httpd_ws_get_fd_info(s_http, fds[i]) != HTTPD_WS_CLIENT_WEBSOCKET || !session ||
                (out->route >= 0 && session->route != out->route)) continue;
            httpd_ws_frame_t frame = {.type = HTTPD_WS_TYPE_TEXT, .payload = (uint8_t *)out->text,
                                      .len = strlen(out->text)};
...
static cJSON *response(const cJSON *req, int code) {
    cJSON *o = cJSON_CreateObject();
    cJSON_AddStringToObject(o, "type", "res");
    copy_field(o, req, "reqId"); copy_field(o, req, "op"); copy_field(o, req, "deviceId");
    copy_field(o, req, "service"); copy_field(o, req, "char");
    cJSON_AddNumberToObject(o, "code", code);
    cJSON_AddStringToObject(o, "message", error_text(code));
    timestamp(o);
    return o;
}
static void reply(protocol_request_t *r, cJSON *o) {
    ...
    for (unsigned i = 1; s_cache_bytes + cache->bytes > CACHE_BYTES && i < CACHE_COUNT; ++i) {
        ...
        s_cache_bytes -= old->bytes; cJSON_free(old->response); memset(old, 0, sizeof(*old));
    }
    if (cache->bytes > CACHE_BYTES) { cJSON_free(cache->response); cache->response = NULL; cache->bytes = 0; }
    s_cache_bytes += cache->bytes;
    send_message(o, r->client, r->route, 1);
    cJSON_Delete(o);
    cJSON_Delete(r->request); cJSON_Delete(r->results);
    memset(r, 0, sizeof(*r));
}
...
static bool add_read_value(cJSON *out, const cJSON *request, const char *hex) {
    ...
    uint8_t bytes[VALUE_BYTES + 1]; size_t len = strlen(hex) / 2;
    ...
    bytes[len] = 0;
    if (eq(format, "base64")) {
        unsigned char encoded[(VALUE_BYTES + 2) / 3 * 4 + 1]; size_t encoded_len;
        ...
        for (size_t i = 0; i < len;) {
            uint32_t cp = bytes[i++]; unsigned continuation = 0; uint32_t minimum = 0;
            if (!cp) return false;
            if (cp >= 0xc2 && cp <= 0xdf) { continuation = 1; minimum = 0x80; cp &= 0x1f; }
            else if (cp >= 0xe0 && cp <= 0xef) { continuation = 2; minimum = 0x800; cp &= 0x0f; }
            else if (cp >= 0xf0 && cp <= 0xf4) { continuation = 3; minimum = 0x10000; cp &= 7; }
...
static void complete(protocol_request_t *r, int code, int native, const char *value) {
    ...
    r->value_index = -1;
    if (!eq(str(r->request, "op"), "batch")) {
        cJSON *o = response(r->request, code);
        if (value && !add_read_value(o, r->request, value)) {
            cJSON_Delete(o); o = response(r->request, 1003);
        }
    ...
    if (code) cJSON_AddStringToObject(result, "message", error_text(code));
    ...
    if (code && (stop || code == 4901 || code == 3101 || code == 3003)) {
        while ((step = current(r))) {
            result = cJSON_CreateObject(); copy_field(result, step, "id");
            cJSON_AddNumberToObject(result, "code", 4999);
            cJSON_AddStringToObject(result, "message", "skipped");
            cJSON_AddItemToArray(r->results, result); ++r->step;
        }
    }
    if (!current(r)) {
        cJSON *o = response(r->request, r->first_error ? 4003 : 0);
        cJSON_AddItemToObject(cJSON_AddObjectToObject(o, "data"), "steps", r->results);
        r->results = NULL; reply(r, o);
    }
}
...
static bool normalize_uuid(const char *in, char out[37]) {
    ...
    out[j] = 0;
    if (j == 32 && !strcmp(out + 8, "00001000800000805f9b34fb")) { out[8] = 0; j = 8; }
...
static int device_by_mac(const char *mac, bool create, int addr_type) {
    ...
    for (int i = 0; i < DEVICE_COUNT; ++i) {
        ...
        if (!s_devices[i].used) { if (free_slot < 0) free_slot = i; continue; }
        // 只有扫描发现的临时设备可以被回收，用户登记过的设备始终保留
        if (!s_devices[i].registered && !s_devices[i].quarantine && !device_referenced(i) &&
            (evict < 0 || s_devices[i].seen < s_devices[evict].seen)) evict = i;
    }
    if (!create) return -1;
    ...
    memset(d, 0, sizeof(*d));
    d->used = true; memcpy(d->addr, addr, 6); d->addr_type = addr_type;
    gw_address_format(addr, d->id); strcpy(d->connection, "disconnected"); d->rssi = 127;
    return slot;
}
static protocol_value_t *value_slot(int device, const char *svc, const char *chr, bool create) {
    ...
    if (!v) return NULL;
    memset(v, 0, sizeof(*v));
    v->used = true; v->device = device; v->used_ms = monotonic_ms();
...
bool gw_protocol_owns(const uint8_t addr[6]) {
    bool found = false;
    portENTER_CRITICAL(&s_owner_lock);
    for (unsigned i = 0; i < s_owned_count; ++i) if (!memcmp(s_owned[i], addr, 6)) { found = true; break; }
    portEXIT_CRITICAL(&s_owner_lock);
    return found;
}
static cJSON *device_json(int i, bool delta) {
    ...
    char mac[18]; gw_address_format(d->addr, mac);
    ...
    for (unsigned j = 0; j < VALUE_COUNT; ++j) {
        ...
        cJSON_ArrayForEach(candidate, services) if (eq(str(candidate, "uuid"), v->service)) { svc = candidate; break; }
        if (!svc) {
            svc = cJSON_CreateObject(); cJSON_AddStringToObject(svc, "uuid", v->service);
            cJSON_AddArrayToObject(svc, "chars"); cJSON_AddItemToArray(services, svc);
        }
        cJSON *ch = cJSON_CreateObject();
        cJSON_AddStringToObject(ch, "uuid", v->characteristic); cJSON_AddStringToObject(ch, "value", v->value);
...
static void flush_state(void) {
    ...
    ++s_revision;
    cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "state");
    timestamp(o); cJSON_AddItemToObject(o, "data", state_json(true));
    send_message(o, NULL, ROUTE_BROADCAST, 0); cJSON_Delete(o);
...
static int validate_step(cJSON *r) {
    ...
    const char *op = str(r, "op"); const cJSON *data = data_of(r);
    if (!op || (data && !cJSON_IsObject(data)) || !valid_timeout(r, "timeout") || !valid_timeout(r, "queueTimeout")) return 1003;
    if (!valid_bool(data, "autoConnect") || !valid_bool(data, "enabled") || !optional_string(r, "format") ||
        ...
        !optional_string(data, "policy")) return 1003;
    if (eq(op, "read") || eq(op, "write") || eq(op, "subscribe")) {
        ...
        if (eq(op, "write")) {
            ...
            uint8_t bytes[VALUE_BYTES]; size_t len = 0;
            if (!format || eq(format, "hex")) {
                len = strlen(value);
            ...
            if (eq(format, "utf8")) {
                len = strlen(value); if (len > sizeof(bytes)) return 1003; memcpy(bytes, value, len);
            } else if (mbedtls_base64_decode(bytes, sizeof(bytes), &len, (const unsigned char *)value, strlen(value))) return 1003;
        ...
        if (eq(op, "subscribe")) {
            ...
            if ((mode && !eq(mode, "auto") && !eq(mode, "notify") && !eq(mode, "indicate")) ||
                (delivery && !eq(delivery, "latest") && !eq(delivery, "stream"))) return 1003;
        }
        return 0;
    }
    ...
    if (eq(op, "connect") || eq(op, "disconnect")) {
        ...
        return (!policy || eq(policy, "queue") || eq(policy, "reject")) && valid_timeout(data, "queueTimeout") ? 0 : 1003;
    }
    return 1002;
}
...
static void accept_message(const cJSON *source, const char *client, int route) {
    ...
    if (!cJSON_IsObject(source) || !eq(str(source, "type"), "req") || !id || !*id || strlen(id) >= ID_BYTES) {
        cJSON *o = response(source, 1001); send_message(o, client, route, 1); cJSON_Delete(o); return;
    }
    for (unsigned i = 0; i < CACHE_COUNT; ++i) if (s_cache[i].used && eq(client, s_cache[i].client) && eq(id, s_cache[i].req_id)) {
        cJSON *cached = s_cache[i].response ? cJSON_Parse(s_cache[i].response) : response(source, 2001);
        send_message(cached, client, route, 1);
        cJSON_Delete(cached);
        return;
    }
    for (unsigned i = 0; i < REQUEST_COUNT; ++i) if (s_requests[i].used && eq(client, s_requests[i].client) && eq(id, str(s_requests[i].request, "reqId"))) {
        // 同一客户端重连时将最终响应路由到最近一次请求来源
        s_requests[i].route = route; return;
    }
    protocol_request_t *r = NULL;
    for (unsigned i = 0; i < REQUEST_COUNT; ++i) if (!s_requests[i].used) { r = &s_requests[i]; break; }
    if (!r) { cJSON *o = response(source, 2001); send_message(o, client, route, 1); cJSON_Delete(o); return; }
    r->request = cJSON_Duplicate(source, true);
    if (!r->request) return;
    r->used = true; r->route = route; r->device = -1; r->value_index = -1;
    r->sequence = ++s_sequence; r->queued = monotonic_ms();
    snprintf(r->client, sizeof(r->client), "%s", client);
    const char *op = str(r->request, "op");
    const cJSON *data = data_of(r->request);
    if (!op || (data && !cJSON_IsObject(data)) || !valid_timeout(r->request, "timeout") || !valid_timeout(r->request, "queueTimeout")) {
        reply(r, response(r->request, 1003)); return;
    }
    if (eq(op, "manage")) {
        cJSON *result = NULL;
    ...
    if (eq(op, "snapshot")) {
        flush_state();
        cJSON *o = response(r->request, 0); cJSON_AddItemToObject(o, "data", state_json(false)); reply(r, o); return;
    }
    if (eq(op, "scan")) {
        const char *action = str(data, "action");
        if ((!eq(action, "start") && !eq(action, "stop")) || !valid_timeout(data, "duration") || !valid_bool(data, "active")) {
            reply(r, response(r->request, 1003)); return;
        }
        uint8_t addr[6] = {0}; bool pending = false; uint16_t handle = 0; int native = 0;
    ...
    if (eq(op, "batch")) {
        ...
        if (!cJSON_IsArray(steps) || !cJSON_GetArraySize(steps) || cJSON_GetArraySize(steps) > MAX_STEPS ||
            !valid_bool(data, "stopOnError") || !valid_bool(data, "autoConnect")) code = 1003;
        else cJSON_ArrayForEach(step, steps) { code = validate_step(step); if (code) break; }
        r->results = cJSON_CreateArray();
        if (!r->results) code = 2001;
    } else code = validate_step(r->request);
    if (code) { reply(r, response(r->request, code)); return; }
    if (!optional_string(r->request, "mac") || !optional_string(data, "mac") ||
        ...
        !optional_string(data, "deviceUuid")) { reply(r, response(r->request, 1003)); return; }
    const char *device_uuid = str(data, "deviceUuid");
    if (device_uuid) {
        char uuid[37];
        if (strlen(device_uuid) != 36 || !normalize_uuid(device_uuid, uuid)) { reply(r, response(r->request, 1003)); return; }
    }
    const char *device_id = str(r->request, "deviceId");
    if (!device_id || !*device_id || strlen(device_id) >= ID_BYTES) { reply(r, response(r->request, 1003)); return; }
    int device = find_device(device_id);
    if (device < 0) { reply(r, response(r->request, 3001)); return; }
    // 只有用户在网关登记过的设备才允许连接和操作，扫描发现的设备仅供展示
    if (!s_devices[device].registered) { reply(r, response(r->request, 3004)); return; }
    ...
    if (addr_type && !eq(addr_type, "public") && !eq(addr_type, "random")) { reply(r, response(r->request, 1003)); return; }
    if (mac) {
        uint8_t addr[6];
        if (!gw_address_parse(mac, addr) || memcmp(addr, s_devices[device].addr, 6)) { reply(r, response(r->request, 1003)); return; }
    }
    if (addr_type) s_devices[device].addr_type = eq(addr_type, "random");
    if (device_uuid) {
        if (s_devices[device].uuid[0] && !eq(s_devices[device].uuid, device_uuid)) { reply(r, response(r->request, 1003)); return; }
...
static void accept_frame(cJSON *frame, int route) {
    ...
    if (!cJSON_IsObject(frame) || !cJSON_IsNumber(version) || version->valuedouble != 1 ||
        ...
        cJSON *o = response(NULL, 1001); send_message(o, NULL, route, 1); cJSON_Delete(o); return;
    }
    cJSON *message;
...
static void update_value(protocol_value_t *v, const char *value) {
    ...
    strcpy(v->value, value); v->ts = gw_protocol_timestamp(); v->valid = true; v->dirty = true;
    s_devices[v->device].seen = v->ts; s_devices[v->device].dirty = true;
}
static void process_event(cJSON *event) {
    const char *type = str(event, "type"), *mac = str(event, "device");
    int device = device_by_mac(mac, eq(type, "ble.scan") || eq(type, "ble.connection"), number(event, "addressType", 0));
    if (device < 0) return;
    ...
    if (eq(type, "ble.subscribe_result") && str(event, "service") && !native) {
        ...
        if (normalize_uuid(str(event, "service"), service) && normalize_uuid(str(event, "char"), characteristic)) {
            ...
            if (v) {
                v->handle = number(event, "handle", 0); v->subscribed = true;
    ...
    if (eq(type, "ble.rssi")) {
        d->rssi = number(event, "rssi", 127); d->seen = gw_protocol_timestamp(); d->dirty = true; return;
    }
    if (eq(type, "ble.scan")) {
        d->rssi = number(event, "rssi", 127); d->seen = gw_protocol_timestamp(); d->addr_type = number(event, "addressType", d->addr_type);
    ...
    if (eq(type, "ble.connection")) {
        ...
        d->dirty = true; d->seen = gw_protocol_timestamp();
        if (!connected) {
            d->quarantine = false; d->quarantine_until = 0;
            for (unsigned i = 0; i < VALUE_COUNT; ++i) if (s_values[i].used && s_values[i].device == device) {
                s_values[i].handle = 0; s_values[i].subscribed = false;
            }
        }
        cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "connection");
        cJSON_AddStringToObject(o, "deviceId", d->id); timestamp(o);
        cJSON *data = cJSON_AddObjectToObject(o, "data"); cJSON_AddStringToObject(data, "state", d->connection);
        copy_field(data, event, "mtu"); copy_field(data, event, "reason");
        if (!connected && (native || number(event, "reason", 0))) {
            cJSON_AddNumberToObject(o, "code", 3101); cJSON_AddStringToObject(o, "message", "disconnected");
        }
        send_message(o, NULL, ROUTE_BROADCAST, 1); cJSON_Delete(o);
    }
    if (eq(type, "ble.notify")) {
        const char *value = str(event, "data");
        if (!value || strlen(value) > VALUE_BYTES * 2) { atomic_fetch_add(&s_overflow, 1); return; }
        ...
        for (unsigned i = 0; i < VALUE_COUNT; ++i) {
            ...
            if (v->subscribed && !v->latest) {
                cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "notify");
                cJSON_AddStringToObject(o, "deviceId", d->id); cJSON_AddStringToObject(o, "service", v->service);
                cJSON_AddStringToObject(o, "char", v->characteristic); cJSON_AddStringToObject(o, "value", v->value);
                cJSON_AddStringToObject(o, "format", "hex"); timestamp(o);
                send_message(o, NULL, ROUTE_STREAM, 1);
                if (s_stream_count < EVENT_COUNT) {
                    s_stream[(s_stream_head + s_stream_count++) % EVENT_COUNT] = o;
                } else {
                    cJSON_Delete(o); atomic_fetch_add(&s_overflow, 1);
                    d->quarantine = true; gw_adapter_disconnect(d->addr);
                }
            }
        }
        // 句柄变化或未订阅导致无法归属的通知要计数，便于排查通知丢失
        if (!matched) atomic_fetch_add(&s_unmatched, 1);
        return;
    }
    for (unsigned i = 0; i < REQUEST_COUNT; ++i) {
        ...
        int code = native ? (eq(op, "read") ? 4101 : eq(op, "write") ? 4201 : eq(op, "subscribe") ? 4301 : connecting ? 3003 : 3101) : 0;
        bool matched = (eq(op, "connect") && eq(type, "ble.services_ready")) ||
                       ...
                       (eq(op, "subscribe") && (eq(type, "ble.subscribe_result") || eq(type, "ble.unsubscribe_result")));
        // 连接建立失败属于连接错误，不应报成“已断开”
        if (disconnected && !eq(op, "disconnect")) { matched = true; code = connecting ? 3003 : 3101; }
        if (!matched) continue;
        if (!disconnected && !eq(op, "connect") && number(event, "handle", 0) != r->handle) continue;
        if (r->auto_connecting) {
            r->auto_connecting = false; r->pending = false;
...
static void schedule(void) {
    ...
    for (unsigned i = 0; i < REQUEST_COUNT; ++i) {
        ...
        if (r->running && (now >= r->deadline || (r->pending && now >= r->step_deadline))) {
            ...
            if (r->pending) {
                d->quarantine = true; d->quarantine_until = now + QUARANTINE_MS;
                gw_adapter_disconnect(d->addr);
                if (active) --active;
            }
            complete(r, connecting ? 3003 : 4901, 0, NULL); continue;
        }
        ...
        if (!r->running) {
            ...
            if (now - r->queued >= timeout) { reply(r, response(r->request, 2002)); continue; }
            bool earlier = false;
            for (unsigned j = 0; j < REQUEST_COUNT; ++j)
                if (s_requests[j].used && s_requests[j].device == r->device && s_requests[j].sequence < r->sequence) earlier = true;
            if (earlier) continue;
        }
        if (d->quarantine || active >= GW_V1_MAX_GATT_OPS) continue;
        cJSON *step = current(r);
        gw_link_t link = {0}; bool linked = gw_adapter_link(d->addr, &link);
        ...
        if (connect && linked && link.connected && link.services_ready) { complete(r, 0, 0, NULL); continue; }
        if (connect && linked) continue;
        if (!connect && !eq(op, "disconnect") && (!link.connected || !link.services_ready)) { complete(r, 3002, 0, NULL); continue; }
        if (eq(op, "disconnect") && !linked) { complete(r, 0, 0, NULL); continue; }
        ...
        if (!connect && (eq(op, "read") || eq(op, "write") || eq(op, "subscribe"))) {
            ...
            if (!v) { complete(r, 2001, 0, NULL); continue; }
            r->value_index = (int)(v - s_values);
        } else r->value_index = -1;
        cJSON *auto_req = NULL;
        if (connect && !eq(op, "connect")) { auto_req = cJSON_CreateObject(); cJSON_AddStringToObject(auto_req, "op", "connect"); }
        bool pending = false; int native = 0;
        int code = gw_protocol_ble_start(auto_req ? auto_req : step, d->addr, d->addr_type, &r->handle, &pending, &native);
        cJSON_Delete(auto_req);
        bool reject = eq(str(data_of(r->request), "policy"), "reject");
        if ((code == 2001 || code == 2003) && !reject) continue;
        if (!r->running) {
            r->running = true; r->started = now;
            r->deadline = now + number(r->request, "timeout", eq(str(r->request, "op"), "batch") ? 10000 : connect ? 15000 : 5000);
        }
        if (!code && pending) {
            r->pending = true; r->auto_connecting = connect && !eq(op, "connect"); ++active;
            r->step_deadline = now + number(step, "timeout", connect ? 15000 : 5000);
            if (v) v->handle = r->handle;
            if (connect || eq(op, "disconnect")) {
                strcpy(d->connection, connect ? "connecting" : "disconnecting"); d->dirty = true;
            }
...
void gw_protocol_receive(const char *text, int len, int route) {
    ...
    if (xQueueSend(s_input, &input, 0) != pdTRUE) { cJSON_Delete(input.json); atomic_fetch_add(&s_overflow, 1); }
}
bool gw_protocol_event(const cJSON *event) {
    ...
    if (!input.json || xQueueSend(s_input, &input, 0) != pdTRUE) {
        cJSON_Delete(input.json); atomic_fetch_add(&s_overflow, 1); return false;
    }
    return true;
}
void gw_protocol_online(void) { s_online = true; }
static void hello(void) {
    cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "hello"); timestamp(o);
    cJSON *data = cJSON_AddObjectToObject(o, "data"); cJSON_AddNumberToObject(data, "protocol", 1);
    cJSON_AddStringToObject(data, "firmware", GW_FIRMWARE_VERSION);
    cJSON_AddBoolToObject(data, "timeSynced", gw_protocol_timestamp() != 0);
    cJSON *ble = cJSON_AddObjectToObject(data, "ble"); cJSON_AddNumberToObject(ble, "maxConnections", 32); cJSON_AddNumberToObject(ble, "maxGattOps", GW_V1_MAX_GATT_OPS);
    ...
    cJSON_AddBoolToObject(features, "batch", true); cJSON_AddBoolToObject(features, "snapshot", true);
    cJSON_AddBoolToObject(features, "scan", true); cJSON_AddBoolToObject(features, "notify", true); cJSON_AddBoolToObject(features, "waitNotify", false);
    // 管理动作列表与 gw_manager_manage 保持一致，App 据此决定显示哪些入口
    const char *actions[] = {"status", "backup", "save", "upsert", "remove", "pause", "resume",
                             "diagnostics", "config.get", "config.set", "restart"};
    cJSON_AddItemToObject(features, "managementActions", cJSON_CreateStringArray(actions, sizeof(actions) / sizeof(actions[0])));
    const char *formats[] = {"hex", "base64", "utf8"}; cJSON_AddItemToObject(data, "formats", cJSON_CreateStringArray(formats, 3));
    cJSON_AddNumberToObject(data, "maxFrameBytes", GW_V1_MAX_FRAME); cJSON_AddNumberToObject(data, "dedupEntries", CACHE_COUNT);
...
void gw_protocol_tick(void) {
    ...
    for (unsigned i = 0; i < EVENT_COUNT && xQueueReceive(s_input, &input, 0) == pdTRUE; ++i) {
        if (input.event) process_event(input.json); else accept_frame(input.json, input.route);
    ...
    if (lost) {
        ...
        for (unsigned i = 0; i < VALUE_COUNT; ++i) {
            ...
            if (!d->quarantine) { d->quarantine = true; d->quarantine_until = monotonic_ms() + QUARANTINE_MS; gw_adapter_disconnect(d->addr); }
        }
        cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "overflow");
        cJSON_AddNumberToObject(o, "code", 2001); cJSON_AddNumberToObject(cJSON_AddObjectToObject(o, "data"), "dropped", lost);
        timestamp(o); send_message(o, NULL, ROUTE_BROADCAST, 1); cJSON_Delete(o);
    }
    schedule();
    for (unsigned i = 0; i < 8 && s_stream_count; ++i) {
        cJSON *o = s_stream[s_stream_head];
        if (!send_message(o, NULL, ROUTE_MQTT, 1)) break;
        cJSON_Delete(o); s_stream[s_stream_head] = NULL;
        s_stream_head = (s_stream_head + 1) % EVENT_COUNT; --s_stream_count;
    }
    int64_t now = monotonic_ms();
    if (now >= s_flush) {
        s_flush = now + 200;
        flush_state();
        if (s_scan_until) {
            cJSON *o = cJSON_CreateObject(); cJSON_AddStringToObject(o, "type", "event"); cJSON_AddStringToObject(o, "op", "scan"); timestamp(o);
            ...
            for (int i = 0; i < DEVICE_COUNT; ++i) if (s_devices[i].scan_dirty) {
                cJSON_AddItemToArray(devices, scan_json(i)); s_devices[i].scan_dirty = false;
            }
...
static esp_err_t ws_handler(httpd_req_t *req) {
    ...
    if (err == ESP_OK) gw_protocol_receive((const char *)frame.payload, frame.len, ((ws_session_t *)req->sess_ctx)->route);
    free(frame.payload); return err;
}
#endif
