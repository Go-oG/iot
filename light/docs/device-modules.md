...
设备接入只有一条链路：一份设备 JSON 创建一个 `Device`，同一个对象描述属性和报文，并负责
连接、下发与上报解析；`DevicePage` 只按属性声明渲染界面。App 里没有按设备类型分叉的 Dart
代码。
...
| 层级 | 实现 | 职责 |
| --- | --- | --- |
| 设备 | `Device` | 唯一的设备类型：解析并校验 JSON、编译帧模板、编解码、连接订阅、FIFO 下发、上报去重与状态维护 |
| 内置定义 | `Device.loadDefinitions()` / `Device.definitionFor()` | 启动时读取 `assets/devices/*.json`，按标识创建 `Device` |
| 动态界面 | `DevicePage` | 按 `ui.renderer` 生成控件，按能力与角色权限决定可用动作，展示上报值、期望值与下发记录 |
...
```json
{
  ...
  "properties": {
    ...
    "characteristic": "8332af20-6d0e-4eea-bb35-665544332211",
    "channels": {
      "name": "五路灯光",
      "type": "object",
      ...
      "write": {
        "op": "write",
        "mode": "withResponse",
        "cmd": "34438888 03 00 ${length:u8} ${value:object,fields=red:u8;green:u8;blue:u8;white:u8;uv:u8} ${crc16modbus}"
      },
      "notify": {
        "op": "subscribe",
        "response": "43348888 03 00 ${skip:u8} 00 ${crc16modbus,at=-2}"
      }
    }
  }
}
...
- 逻辑类型：`bool`、`int`、`double`、`string`、`enum`、`array`、`object`，支持范围、步长、
  枚举、长度、条数与嵌套必填校验
- 请求帧用 `cmd` 描述，响应帧用字符串 `response` 描述；`fields[]` / `order` / `match` 不再出现在模型里
- `properties.service` 与 `properties.characteristic` 是设备级默认值，单个操作可以覆盖
- 写操作使用 `mode` 指定 `withResponse` 或 `withoutResponse`
...
字面量可以连续书写，也可以按空格分组；写回模板时会重新整理为规范间隔，因此模型文件与
编辑器保存的内容不会出现语义漂移（见 `device_frame_test.dart` 的规范化用例）。
...
模板在 `Device.fromJson` 时编译成运行时结构，编码与解码都由同一个 `Device` 执行；
`Device.toJson()` 会把结构写回 `cmd` / `response` 规范模板，因此配置编辑器保存、备份导出都
保持新版协议形式，字段名、跨度、端序都会显式写出。编译期错误带行列位置，覆盖未知字段类型或选项、跨度
引用不存在的字段、字段名重复、长度字段不是固定宽度、校验范围包含自身、
响应模板里动态宽度字段之后缺少 `at=` 等情况。

## 界面渲染器

`DevicePage` 按 `ui.renderer` 生成控件，未声明时按类型推断：
...
- 写入值先按属性声明校验，再由 `Device.encodeWrite` 编码；
...
2. 用假 MQTT 测试覆盖编码字节与响应匹配，参考 [AT5 设备定义测试](../test/device_at5_test.dart)
3. 运行时也可以「设备页 → 编辑设备模型」或「新建 / 导入物模型」写入本机配置，
   同一份 JSON 可导出、备份与分享
4. 需要新的交互形态时，在模型里声明新的 `ui.renderer` 并在 `DevicePage` 实现渲染，
   不要为单台设备写专用页面
