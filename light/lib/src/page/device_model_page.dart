...
/// 由 [DeviceModel] 渲染的设备页
///
/// 通用 UI 渲染：属性按 ui.renderer 生成控件，读写通知能力与角色权限决定可用动作，
/// 上报值、期望值、下发记录都直接来自 [DeviceModelSession]，页面不包含设备私有逻辑。
class DeviceModelPage extends StatefulWidget {
  ...
  final DeviceModelSession session;
...
class _DeviceModelPageState extends State<DeviceModelPage> {
  ...
  DeviceModelSession get _session => widget.session;
  ...
  Future<void> _write(String property, Object? value) =>
      _run(() async => _session.writeProperty(property, value));
  ...
  Future<void> _edit(
    ...
    final value = await showDialog<Object?>(
      context: context,
      builder: (_) => _ModelValueDialog(
        spec: property.value,
        title: '设置${property.name}',
            ...
            children: [
              SurfaceCard(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${model.id} · ${_session.role.wireName}',
                      style: const TextStyle(color: AppColors.muted),
                    ),
                    const SizedBox(height: 8),
                    Text(_session.ready ? '网关在线' : '网关离线'),
                    if (_session.stale && _session.reported.isNotEmpty)
                      const Text(
                        '部分上报已过期或缺少顺序校验',
                    ...
                    Wrap(
                      ...
                      children: [
                        FilledButton(
                          onPressed: _session.ready && !_session.busy
                              ? () => _run(_session.start)
                              : null,
                          child: const Text('连接'),
                        ),
              ...
              if (_error ?? _session.lastError case final String error)
                Padding(
                ...
                if (entry.value.ui?.renderer != UiRenderer.hidden)
                  Padding(
                    ...
                    child: _PropertyCard(
                      ...
                      onEdit: _edit,
                      onRead: (property) =>
                          _run(() => _session.readProperty(property)),
                    ),
                  ),
              ...
              const SizedBox(height: 8),
              const Text(
                '“已写入”表示网关完成 BLE 写入，实际状态以设备上报为准',
                style: TextStyle(fontSize: 12, color: AppColors.muted),
              ),
              if (_session.commands.isEmpty)
                const Text('暂无记录', style: TextStyle(color: AppColors.muted)),
              for (final record in _session.commands.take(20))
                ListTile(
                  contentPadding: EdgeInsets.zero,
                  title: Text('${record.operation} · ${_stateLabel(record.state)}'),
                  subtitle: Text(
                    '${record.requestId ?? record.message ?? ''}\n${record.at.toLocal()}',
                  ),
                ),
            ],
          ),
  ...
  static String _stateLabel(ModelCommandState state) => switch (state) {
    ModelCommandState.written => '已写入',
    ModelCommandState.deviceAck => '设备已确认',
    ModelCommandState.deviceState => '设备已回报',
...
class _PropertyCard extends StatelessWidget {
  ...
  final String id;
  final DevicePropertyModel property;
  final DeviceModelSession session;
  final Future<void> Function(String property, Object? value) onWrite;
  final Future<void> Function(String id, DevicePropertyModel property, Object? current) onEdit;
  final Future<void> Function(String property) onRead;

  bool get _enabled => session.ready && !session.busy;

  Object? get _current {
    final reported = session.reported[id];
    if (reported != null) return reported.value;
    final desired = session.desired[id];
  ...
  @override
  Widget build(BuildContext context) {
    final state = session.reported[id];
    return SurfaceCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          if (state?.sampledAt case final DateTime sampledAt)
            Text(
              '采样于 ${sampledAt.toLocal()}',
              style: const TextStyle(fontSize: 11, color: AppColors.muted),
            ),
          if (session.desired[id] case final Object desired)
            Text(
              '期望值：${_display(desired)} · 尚未回报一致值',
          ...
          if (property.canRead)
            Align(
              ...
              child: TextButton(
                onPressed: _enabled ? () => onRead(id) : null,
                child: const Text('读取'),
              ),
            ),
        ],
  ...
  String get _capabilities {
    ...
    final parts = <String>[
      if (property.canRead) '可读',
      if (property.canWrite) '可写',
      if (property.canNotify) '通知',
    ];
    if (parts.isEmpty) return '不可访问';
    final restricted = <String>[
      if (roles?.read != null && property.canRead)
        '读 ${roles!.read!.map((role) => role.wireName).join('/')}',
      if (roles?.write != null && property.canWrite)
        '写 ${roles!.write!.map((role) => role.wireName).join('/')}',
      if (roles?.notify != null && property.canNotify)
        '通知 ${roles!.notify!.map((role) => role.wireName).join('/')}',
    ];
        ...
        return [
          Row(
            children: [
              ...
              Switch(
                value: _current == true,
                onChanged: writable ? (value) => onWrite(id, value) : null,
              ),
            ],
          ),
        ];
  ...
  static UiRenderer _inferredRenderer(ValueSpec spec) => switch (spec.type) {
    ...
    ValueType.integer || ValueType.decimal => spec.constraints?.min != null &&
        spec.constraints?.max != null
        ? UiRenderer.slider
        : UiRenderer.input,
    _ => UiRenderer.input,
  };
    ...
    return Column(
      ...
      children: [
        Slider(
          ...
          onChangeEnd: writable
              ? (next) => onWrite(id, _number(spec, next))
              : null,
        ),
        Text(
    ...
    return SegmentedButton<Object?>(
      segments: [
        for (final value in spec.values)
          ButtonSegment(value: value.value, label: Text(value.label)),
      ],
  ...
  Widget _stepper(ValueSpec spec, bool writable) {
    ...
    return Row(
      children: [
        IconButton(
          onPressed: writable && value - step >= min
              ? () => onWrite(id, _number(spec, value - step))
              : null,
        ...
        IconButton(
          onPressed: writable && value + step <= max
              ? () => onWrite(id, _number(spec, value + step))
              : null,
    ...
    return [
      for (final entry in spec.properties.entries)
        if (entry.value.type == ValueType.integer ||
            entry.value.type == ValueType.decimal)
          _channel(
            entry.key,
            entry.value,
            (values[entry.key] as num?)?.toDouble() ?? 0,
            writable,
          ),
    ];
  }
      ...
      children: [
        ...
        Expanded(
          child: Slider(
            ...
            onChanged: writable ? (_) {} : null,
            onChangeEnd: writable
                ? (next) => onWrite(id, _channelValue(name, spec, next))
                : null,
          ),
        ),
    ...
    return {
      for (final entry in spec.properties.entries)
        entry.key: entry.key == name
            ...
            : values[entry.key] ??
                  (entry.value.constraints?.min ?? 0),
    };
  }

  static Object _number(ValueSpec spec, double value) =>
      spec.type == ValueType.integer ? value.round() : value;
...
class _ModelValueDialogState extends State<_ModelValueDialog> {
  ...
  Object? _default(ValueSpec spec) => switch (spec.type) {
    ...
    ValueType.integer || ValueType.decimal => spec.constraints?.min ?? 0,
    ValueType.string => '',
    ValueType.enumeration => spec.values.isEmpty ? null : spec.values.first.value,
    ValueType.array => <Object?>[],
    ValueType.object => {
      for (final entry in spec.properties.entries)
        entry.key: _default(entry.value),
    },
  };
  ...
  Widget _field(ValueSpec spec, String label, Object? value, ValueChanged<Object?> changed) {
    if (spec.type == ValueType.enumeration) {
      return DropdownButtonFormField<Object>(
        initialValue: spec.values.any((item) => item.value == value)
            ? value
            : null,
        isExpanded: true,
    ...
    return TextFormField(
      initialValue: value == null
          ...
          ? jsonEncode(value)
          : '$value',
      keyboardType: (spec.type == ValueType.integer ||
              spec.type == ValueType.decimal)
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    actions: [
      TextButton(
        onPressed: () => Navigator.pop(context, _ModelValueDialog.cancelled),
        child: const Text('取消'),
      ),
      FilledButton(
        onPressed: () {
...
/// 由路由打开的通用设备页：设备定义来自本地保存的 [DeviceModel]
class DeviceModelDevicePage extends StatelessWidget {
  const DeviceModelDevicePage({required this.deviceId, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final session = controller.deviceSessionFor(deviceId);
    return DeviceModelPage(
      session: session,
      onDisconnect: () => controller.deviceRegistry.disconnectDevice(deviceId),
      onEditModel: () =>
          context.push('/device-model?deviceId=${Uri.encodeComponent(deviceId)}'),
    );
  }
}
