...
class MqttDebugClient extends ChangeNotifier {
  ...
  /// 自动重连的最短与最长间隔
  static const _retryMin = Duration(seconds: 3);
  ...
  Future<void> _open(MqttConfig settings, {required bool manual}) async {
    ...
    try {
      ...
      _reconnecting = false;
      _retryDelay = _retryMin;
      _log(MqttMsgKind.system, '连接', '服务器已连接');
      ...
      client.disconnect();
      if (manual) {
        _log(MqttMsgKind.system, '连接', '连接失败，请检查服务器、证书和账号');
      }
      rethrow;
    }
  }
  ...
  Future<void> _restoreSubscriptions(MqttServerClient client) async {
    for (final topic in Map.of(_desired).entries) {
      ...
      try {
        ...
        if (_client != client) return;
        _log(MqttMsgKind.system, '订阅', '重新订阅失败，请手动重试', topic: topic.key);
      }
    }
  }
  ...
  void _scheduleRetry(MqttConfig settings) {
    if (!_shouldRetry) return;
    _retryTimer?.cancel();
    _reconnecting = true;
    _log(MqttMsgKind.system, '断开', '连接已中断，${_retryDelay.inSeconds} 秒后自动重连');
  ...
  Future<void> _retry(MqttConfig settings) async {
  ...
  void unsubscribe(String topic) {
    ...
    _desired.remove(topic);
    _log(MqttMsgKind.system, '取消订阅', '已取消订阅', topic: topic);
  }
  ...
  void disconnect() {
    ...
    _desired.clear();
    client?.disconnect();
    if (client != null) _log(MqttMsgKind.system, '断开', '调试连接已关闭');
  }
